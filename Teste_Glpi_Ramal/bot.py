...
class Bot:
    def bot(self):
        ...
        webBot.browse("http://10.2.17.7/zabbix/zabbix.php?action=host.list&filter_groups%5B%5D=33&filter_host=&filter_dns=&filter_ip=&filter_port=&filter_status=0&filter_monitored_by=-1&filter_evaltype=0&filter_tags%5B0%5D%5Btag%5D=&filter_tags%5B0%5D%5Boperator%5D=0&filter_tags%5B0%5D%5Bvalue%5D=&filter_set=1")
        ...
        banco_ramal = Webscrap().webscrap(inBot=webBot, inXPATH="/html/body/div/main/form/table", inLines=0,inNext='', inGetLink=False)

        # Sequence: Lista_acoes

        # Assign Activity
        # Displayname: Assign
        banco_ramal = banco_ramal.assign(Interface2=banco_ramal['Interface'].str.split(':').str[0])

        # Assign Activity
        # Displayname: Assign_Values
        lista_ips = banco_ramal['Interface2'].tolist()

        # Assign Activity
        # Displayname: Assign_Values
        banco_ramal = banco_ramal.assign(Localizacao=banco_ramal['Nome'].str.split(' - ').str[1])

        # Assign Activity
        # Displayname: Assign_Values
        lista_localizacoes = banco_ramal['Localizacao'].tolist()

        # Assign Activity
        # Displayname: Converte_dados
        banco_ramal = banco_ramal.to_dict(orient='records')
        ...
        for item_ramal in banco_ramal:
            ...
            nome_impressora.send_keys(item_ramal["Nome"])
            ...
            end_ip.send_keys(item_ramal["Interface2"])
